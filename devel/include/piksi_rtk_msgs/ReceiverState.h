  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::piksi_rtk_msgs::ReceiverState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.sat.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.sat[i]);
    }
    if (v.sat.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.cn0.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.cn0[i]);
    }
    if (v.cn0.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "tracking_running: ";
    if (v.tracking_running.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.tracking_running.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.tracking_running[i]);
    }
    if (v.tracking_running.empty() || true)
      s << "]";
    if (true || !indent.empty())
